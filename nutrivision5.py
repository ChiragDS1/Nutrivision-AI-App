...
def main():
    ...
    page = st.sidebar.selectbox("Go to", ["Dashboard", "User Profile", "Diet Plan", "Workout Plan", "Freshness Checker", "Dish Identifier", "Logout"])
...
def dashboard(user_id):
    ...
    row = c.fetchone()
    if row:
        labels = ["Name", "Gender", "Body Type", "Activity Level", "BMI", "Goal", "Weight Loss Rate", "Workout Type", "Gym Focus"]
        st.subheader("Latest Profile Information")
        for i, label in enumerate(labels):
            st.markdown(f"**{label}:** {row[i+1] if row[i+1] else 'N/A'}")

        # --- CHART: BMI trend ---
        c.execute('SELECT created_at, bmi FROM profiles WHERE user_id=? AND created_at IS NOT NULL ORDER BY created_at', (user_id,))
        data = c.fetchall()
        if data:
            dates, bmis = zip(*data)
            df = pd.DataFrame({"Date": dates, "BMI": bmis})
            st.subheader("BMI Trend")
            fig, ax = plt.subplots()
            df.plot(x="Date", y="BMI", ax=ax, marker='o', color='green')
            ax.set_ylabel("BMI")
            ax.set_title("BMI Over Time")
            st.pyplot(fig)

        # --- CHART: Activity Level Summary ---
        ...
        if activity_data:
            ...
            st.subheader("Activity Level Frequency")
            fig, ax = plt.subplots()
...
def get_profile_defaults(profile):
    ...
    return {
        ...
        'body_type': profile[3],
        'activity': profile[4],
        'height': round((profile[5] ** 0.5), 2) if profile[5] else 0.0,
        'weight': round(profile[5] * (profile[5] ** 0.5) ** 2, 2) if profile[5] else 0.0,
        'goal': profile[6],
        'weight_loss_rate': profile[7] if profile[7] else "0.5 kg/week",
        'workout_type': profile[8],
        'gym_focus': profile[9] if profile[9] else "Cardio Heavy"
    }
...
def profile_page(user_id):
    ...
    name = st.text_input("Full Name", value=prev_profile[1] if prev_profile else "")
    gender = st.selectbox("Gender", ["Male", "Female", "Other"], index=["Male", "Female", "Other"].index(prev_profile[2]) if prev_profile else 0)
    body_type = st.selectbox("Body Type", ["Ectomorph : Lean Body", "Mesomorph : Average Body", "Endomorph : Bulky or Fat"], index=["Ectomorph : Lean Body", "Mesomorph : Average Body", "Endomorph : Bulky or Fat"].index(prev_profile[3]) if prev_profile else 0)
    activity = st.selectbox("Physical Activity Level", ["Low: 1-2 days a week", "Moderate: 3-5 days a week", "High: Almost Everyday"], index=["Low: 1-2 days a week", "Moderate: 3-5 days a week", "High: Almost Everyday"].index(prev_profile[4]) if prev_profile else 0)
    height = st.number_input("Height (m)", value=round((prev_profile[5] ** 0.5), 2) if prev_profile and prev_profile[5] else 0.0)
    weight = st.number_input("Weight (kg)", value=0.0)
    goal = st.selectbox("Fitness Goal", ["Lose Fat", "Gain Muscle", "Maintain"], index=["Lose Fat", "Gain Muscle", "Maintain"].index(prev_profile[6]) if prev_profile else 0)

    weight_loss_rate = ""
    if goal == "Lose Fat":
        weight_loss_rate = st.selectbox("How fast do you want to lose weight?", ["0.5 kg/week", "0.8 kg/week", "1.0 kg/week"], index=["0.5 kg/week", "0.8 kg/week", "1.0 kg/week"].index(prev_profile[7]) if prev_profile and prev_profile[7] else 0)

    workout_type = st.selectbox("Do you workout in a gym or do you prefer bodyweight exercises?", ["Gym", "Bodyweight"], index=["Gym", "Bodyweight"].index(prev_profile[8]) if prev_profile else 0)
    gym_focus = ""
    if workout_type == "Gym":
        gym_focus = st.selectbox("What kind of workout do you prefer at the gym?", ["Cardio Heavy", "Strength Training Focused", "Mix of Both"], index=["Cardio Heavy", "Strength Training Focused", "Mix of Both"].index(prev_profile[9]) if prev_profile and prev_profile[9] else 0)
    ...
    if st.button("Save Profile"):
        if not all([name, gender, body_type, activity, height, weight, goal, workout_type]) or (goal == "Lose Fat" and not weight_loss_rate) or (workout_type == "Gym" and not gym_focus):
            st.error("Please fill out all required fields before saving your profile.")
        else:
            c.execute('INSERT INTO profiles (user_id, name, gender, body_type, activity_level, bmi, goal, weight_loss_rate, workout_type, gym_focus) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)',
                      (user_id, name, gender, body_type, activity, bmi, goal, weight_loss_rate, workout_type, gym_focus))
            conn.commit()
...
def show_diet_plan(user_id):
    ...
    regenerate = st.button("Regenerate Diet Plan")
...
def show_workout_plan(user_id):
    ...
    regenerate = st.button("Regenerate Workout Plan")
